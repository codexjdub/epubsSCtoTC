...
(function (App) {
  ...
  /* Ids are unique per document in the archive and merely common across it --
   * Calibre stamps calibre_pb_0 into every file it splits, ten times over in
   * the sample. Collapsing the book into one document turns that into ten
   * elements sharing an id, where a link can only ever reach the first. The
   * later ones are renamed, and the map is kept so links from the chapter that
   * owned them follow. */
  function claimIds(doc, claimed) {
    var map = {};
    var all = doc.getElementsByTagName('*');
    for (var i = 0; i < all.length; i++) {
      var id = all[i].getAttribute && all[i].getAttribute('id');
      if (!id) continue;
      if (!claimed[id]) { claimed[id] = true; map[id] = id; continue; }
      var n = 2;
      var candidate = id + '-' + n;
      while (claimed[candidate]) candidate = id + '-' + (++n);
      claimed[candidate] = true;
      map[id] = candidate;
      all[i].setAttribute('id', candidate);
    }
    return map;
  }

  function rewriteLinks(doc, basePath, sectionFor, renames) {
    ...
    for (var i = 0; i < anchors.length; i++) {
      ...
      /* Through the target document's own renaming, so a link into a chapter
       * whose id was a duplicate still reaches that chapter's copy. */
      var renamed = renames[target] && renames[target][fragment];
      if (renamed) anchors[i].setAttribute('href', '#' + renamed);
  ...
  async function toHtml(book) {
    ...
    var claimed = {};
    var renames = {};
    /* Section ids are claimed first so a book cannot contain one already. */
    Object.keys(sectionFor).forEach(function (path) { claimed[sectionFor[path]] = true; });
    for (var d = 0; d < paths.length; d++) {
      var parsed = await documentFor(book, paths[d]);
      renames[paths[d]] = claimIds(parsed, claimed);
      docs.push(parsed);
    }
    ...
    for (var i = 0; i < paths.length; i++) {
      ...
      rewriteLinks(doc, paths[i], sectionFor, renames);
      var body = bodyOf(doc);
      var inner = '';
      ...
      /* Already made unique above, so it can be emitted as it stands. */
      var bodyId = body.getAttribute && body.getAttribute('id');
      var anchor = bodyId ? '<span id="' + escapeHtml(bodyId) + '"></span>\n' : '';
